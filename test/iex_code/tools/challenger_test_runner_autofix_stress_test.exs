defmodule IexCode.Tools.ChallengerTestRunnerAutoFixStressTest do
  ...
  # ============================================================================
  # 4. LiveView Visual Test Runner & AutoFix Studio Lifecycle
  # ============================================================================
  describe "LiveView Test Runner & AutoFix Studio Lifecycle" do
    setup %{workspace_path: path} do
      demo_file = "lib/demo_calc.ex"
      demo_test = "test/demo_calc_test.exs"

      workspace_write_file(
        path,
        demo_file,
        "defmodule DemoCalc do\n  def double(x, unused_scale), do: x * 2\nend\n"
      )

      workspace_write_file(
        path,
        demo_test,
        "defmodule DemoCalcTest do\n  use ExUnit.Case\n  test \"double\", do: assert DemoCalc.double(3, 1) == 6\nend\n"
      )

      {:ok, %{demo_file: demo_file, demo_test: demo_test}}
    end

    test "triggers test run modes (all, failed, stale) and streams progress", %{
      conn: conn,
      workspace_path: path
    } do
      ...
      assert has_element?(view, "#test-runner-panel")

      # Progress streaming updates
      send(view.pid, {:test_runner_progress, 50, "Compiling project..."})
      assert render(view) =~ "50%"
      assert render(view) =~ "Compiling project..."

      # 2. Run failed tests
      render_click(view, "run_tests", %{"mode" => "failed"})
      assert has_element?(view, "#test-runner-panel")

      # 3. Run stale tests
      render_click(view, "run_tests", %{"mode" => "stale"})
      assert has_element?(view, "#test-runner-panel")
    end
  end
end
