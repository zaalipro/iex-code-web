defmodule IexCodeWeb.WorkspaceLiveSignalFoundryCalendarTest do
  ...
  test "calendar visual contracts use neutral tokens, readable type, coarse targets and safe areas" do
    ...
    assert css =~ "repeat(7, minmax(46px, 1fr))"
    assert css =~ "minmax(22.25rem, 1fr) minmax(13rem, 0.65fr)"
    assert css =~ "overscroll-behavior-inline: contain"
    assert css =~ ".calendar-desktop-layout"
