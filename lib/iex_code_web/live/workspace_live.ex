defmodule IexCodeWeb.WorkspaceLive do
  ...
  def mount(params, _session, socket) do
    ...
    research_runs = research_runs(durable_runs)
    ...
    ready_research_results =
      ResearchResults.list_ready(session_id: session.id)
      |> scoped_research_results(research_runs)
    ...
    socket =
      ...
      |> assign(:commit_form, to_form(%{"message" => ""}))
    ...
    summary_step_summaries =
      if socket.assigns.live_action == :research and selected_research_run do
        %{selected_research_run.id => selected_research_steps}
      else
        %{}
      end
    ...
    socket =
      ...
      |> refresh_run_summary_facts(durable_runs, ready_research_results,
        step_summaries: summary_step_summaries
      )
  ...
  def handle_params(_params, uri, socket) do
    ...
    case normalize_workspace_view(query_params, socket.assigns.live_action, route_context) do
      {:replace, path} ->
        case validate_workspace_target(path_params, socket) do
          :ok ->
            {:noreply,
             socket
      ...
      {:ok, active_view} ->
        case validate_workspace_target(path_params, socket) do
          :ok ->
            activate_workspace_params(active_view, route_context, path_params, socket)
  ...
  defp validate_workspace_target(%{"id" => id}, socket)
       when is_binary(id) and id != socket.assigns.session.id do
    with %Sessions.Session{} = session <- fetch_session(id),
         %Projects.Project{} <- fetch_project(session.project_id) do
      :ok
    else
      nil -> {:error, "Session not found"}
      _ -> {:error, "Project for this session was not found"}
    end
  end

  defp validate_workspace_target(_params, _socket), do: :ok

  defp activate_workspace_params(active_view, route_context, path_params, socket) do
    previous_view = socket.assigns.active_view
    previous_route = socket.assigns.workspace_route
    ...
    handle_workspace_params(
      path_params,
      socket,
      {previous_view, previous_route, previous_tab}
    )
  end

  defp handle_workspace_params(params, socket, {previous_view, previous_route, previous_tab}) do
    if params["id"] && params["id"] != socket.assigns.session.id do
      ...
      case fetch_session(params["id"]) do
        nil ->
          {:noreply,
           socket
           |> restore_workspace_identity(previous_view, previous_route, previous_tab)
          ...
          case fetch_project(new_session.project_id) do
            nil ->
              {:noreply,
               socket
               |> restore_workspace_identity(previous_view, previous_route, previous_tab)
               |> put_flash(:error, "Project for this session was not found")}
            ...
            project ->
              ...
              socket =
                ...
                |> rebuild_instrument_summaries()
                |> maybe_request_transition_git_refresh()
  ...
  defp restore_workspace_identity(socket, view, route, tab) do
    socket
    |> assign(:active_view, view)
    |> assign(:workspace_route, route)
  ...
  @impl true
  def handle_event("switch_changes_subtab", %{"tab" => tab}, socket)
      when tab in ["changes", "all", "desktop"] do
    {:noreply, assign(socket, :changes_subtab, tab)}
  end

  def handle_event("switch_changes_subtab", _params, socket), do: {:noreply, socket}
  ...
  @impl true
  def handle_event("set_diff_mode", %{"mode" => mode}, socket) when mode in ["inline", "split"] do
    {:noreply, assign(socket, :diff_mode, mode)}
  end

  def handle_event("set_diff_mode", _params, socket), do: {:noreply, socket}
  ...
  def handle_event("accept_hunk", %{"file" => file, "hunk_id" => hunk_id}, socket) do
    with {:ok, accepted_path} <- authorize_hunk(socket, :unstaged, file, hunk_id),
      ...
      case with_ui_mutation_lock(socket, fn ->
             with {:ok, snapshot} <-
                    fresh_hunk_snapshot(socket, :unstaged, accepted_path, hunk_id),
        ...
        {:ok, _} ->
          {:noreply,
           socket
           |> refresh_git_state()
           |> put_flash(:info, "Accepted hunk #{hunk_id} for #{accepted_path}")}
  ...
  @impl true
  def handle_event("reject_hunk", params, socket) when map_size(params) == 0,
    do: confirm_git_hunk(socket, :discard_git_hunk)

  def handle_event("reject_hunk", _params, socket), do: {:noreply, socket}

  @impl true
  def handle_event("revert_hunk", params, socket) when map_size(params) == 0,
    do: confirm_git_hunk(socket, :revert_git_hunk)
    ...
    with false <- socket.assigns.diff_truncated?,
      ...
      case with_ui_mutation_lock(socket, fn ->
             if fresh_status_authorizes?(root, :unstaged, accepted_path) do
               ...
               if truncated? or String.trim(fresh_text) == "" or fresh_text != expected_diff,
        ...
        {:ok, _} ->
          {:noreply,
           socket
           |> refresh_git_state()
           |> put_flash(:info, "Staged all changes for #{accepted_path}")}
  ...
  def handle_event("revert_file", %{"file" => _file}, socket),
    do: compatibility_git_guard(socket, "Confirmation required before reverting a file")

  def handle_event("revert_file", _params, socket), do: {:noreply, socket}
  ...
  def handle_event("request_revert_git_file", _params, socket),
    do: {:noreply, assign(socket, :pending_git_confirmation, nil)}

  @impl true
  def handle_event("request_discard_git_hunk", %{"file" => file, "hunk_id" => hunk_id}, socket) do
    open_git_hunk_confirmation(socket, :discard_git_hunk, file, hunk_id)
  end

  def handle_event("request_discard_git_hunk", _params, socket),
    do: {:noreply, assign(socket, :pending_git_confirmation, nil)}

  @impl true
  def handle_event("request_revert_git_hunk", %{"file" => file, "hunk_id" => hunk_id}, socket) do
    open_git_hunk_confirmation(socket, :revert_git_hunk, file, hunk_id)
  end

  def handle_event("request_revert_git_hunk", _params, socket),
    do: {:noreply, assign(socket, :pending_git_confirmation, nil)}

  @impl true
  def handle_event("cancel_git_confirmation", _params, socket),
    do: {:noreply, assign(socket, :pending_git_confirmation, nil)}
      ...
      case with_ui_mutation_lock(socket, fn ->
        ...
        {:error, reason} ->
          {:noreply,
           socket
           |> assign(:pending_git_confirmation, nil)
           |> put_flash(:error, "Failed to switch branch: #{ui_mutation_error(reason)}")}
      end
    else
      {:noreply, socket}
    end
  ...
  @impl true
  def handle_event("create_git_branch", params, %{assigns: %{active_view: "changes"}} = socket) do
        ...
        case with_ui_mutation_lock(socket, fn -> Git.create_branch(root, name) end) do
          ...
          {:error, reason} ->
            {:noreply,
             socket
             |> assign(:pending_git_confirmation, nil)
             |> put_flash(:error, "Failed to create branch: #{ui_mutation_error(reason)}")}
        end
    end
  end
      ...
      case with_ui_mutation_lock(socket, fn -> Git.pull(root) end) do
        {:ok, _} ->
          {:noreply,
           ...
           |> assign(:pending_git_confirmation, nil)
           |> refresh_git_state()
           |> put_flash(:info, "Pulled latest changes from remote")}
  ...
  def handle_event("stage_file", %{"file" => file}, socket) do
    with {:ok, accepted_path} <- authorize_status_path(socket, file, [:unstaged, :untracked]) do
      root = socket.assigns.project.root_path

      scope =
        if accepted_path in status_paths(socket.assigns.git_status, :untracked),
          do: :untracked,
          else: :unstaged

      case with_ui_mutation_lock(socket, fn ->
             if fresh_status_authorizes?(root, scope, accepted_path),
               do: Git.stage(accepted_path, root),
      ...
      _ -> compatibility_git_guard(socket, "Staging requires a current Change Ledger snapshot")
    end
  end

  @impl true
  def handle_event("unstage_file", %{"file" => file}, socket) do
    with {:ok, accepted_path} <- authorize_status_path(socket, file, [:staged]) do
      root = socket.assigns.project.root_path

      case with_ui_mutation_lock(socket, fn ->
             if fresh_status_authorizes?(root, :staged, accepted_path),
               do: Git.unstage(accepted_path, root),
               else: {:error, :stale_git_snapshot}
           end) do
        :ok ->
          {:noreply, refresh_git_state(socket)}
      ...
      _ -> compatibility_git_guard(socket, "Unstaging requires a current Change Ledger snapshot")
    end
  end
  ...
  def handle_event("unstage_all", _params, socket), do: {:noreply, socket}
  ...
  def handle_event("unstage_hunk", %{"file" => file, "hunk_id" => hunk_id}, socket) do
    with {:ok, accepted_path} <- authorize_hunk(socket, :staged, file, hunk_id),
      ...
      case with_ui_mutation_lock(socket, fn ->
             with {:ok, snapshot} <-
                    fresh_hunk_snapshot(socket, :staged, accepted_path, hunk_id),
                  true <- snapshot.fingerprint == expected_fingerprint do
        ...
        {:error, reason} ->
          {:noreply,
           put_flash(socket, :error, "Unstage hunk failed: #{ui_mutation_error(reason)}")}
      end
    else
      _ -> {:noreply, socket}
    end
  ...
  def handle_event("update_commit_message", params, socket) do
    ...
    {:noreply,
     socket
     |> assign(:commit_message, msg)
     |> assign(:commit_form, to_form(%{"message" => msg}))}
  end
  ...
  @impl true
  def handle_event("git_commit", _params, %{assigns: %{active_view: "changes"}} = socket) do
    root = socket.assigns.project.root_path
  ...
  def handle_event("git_commit", _params, socket), do: {:noreply, socket}
      ...
      true ->
        ...
        case Router.route(intent, context) do
          {:ok, %{run: run}} ->
            ...
            {:noreply,
             socket
             |> select_research_projection(run)
    ...
    case safe_get_run(run_id) do
      ...
      when session_id == socket.assigns.session.id ->
        {:noreply,
         socket
         |> select_research_projection(run)
         |> push_patch(to: research_path(socket.assigns.workspace_route))}
  ...
  def handle_event("open_research_run", _params, socket),
  ...
  def handle_event(
        "refresh_git_summary",
        _params,
        %{assigns: %{active_view: "changes"}} = socket
      ),
      do: {:noreply, socket}
  ...
  def handle_event("refresh_git_state", _params, %{assigns: %{active_view: "changes"}} = socket),
    do: {:noreply, socket |> assign(:pending_git_confirmation, nil) |> refresh_git_state()}
  ...
  def handle_info(
    ...
    {:noreply, refresh_research_evidence(socket)}
  end
  ...
  def handle_info({:run_created, run}, socket) do
    if socket.assigns.active_view == "research" do
      {:noreply, refresh_research_run_notification(socket, Map.get(run, :id), :created)}
    else
  ...
  def handle_info({:run_updated, run}, socket) do
    if socket.assigns.active_view == "research" do
      {:noreply, refresh_research_run_notification(socket, Map.get(run, :id), :updated)}
    else
      if run.session_id == socket.assigns.session.id do
  ...
  def handle_info({:run_event, event}, socket) do
    if socket.assigns.active_view == "research" do
      {:noreply, refresh_research_run_notification(socket, Map.get(event, :run_id), :progress)}
    else
  ...
  def handle_info({event_name, entity}, socket)
    ...
    cond do
      socket.assigns.active_view == "research" ->
        {:noreply, refresh_research_run_notification(socket, Map.get(entity, :run_id), :progress)}
  ...
  def handle_info({event_name, agent}, socket)
    ...
    if socket.assigns.active_view == "research" do
      {:noreply, refresh_research_run_notification(socket, Map.get(agent, :run_id), :progress)}
    else
  ...
  def handle_info({event_name, control}, socket)
    ...
    if socket.assigns.active_view == "research" do
      {:noreply, refresh_research_run_notification(socket, Map.get(control, :run_id), :progress)}
    else
  ...
  defp maybe_request_transition_git_refresh(%{assigns: %{active_view: "changes"}} = socket),
    do: socket

  defp maybe_request_transition_git_refresh(socket), do: request_deck_git_refresh(socket)
  ...
  defp complete_deck_git_refresh(socket, generation, {project_id, result}) do
    ...
    socket =
      if owns? and project_id == socket.assigns.project.id and
           socket.assigns.active_view != "changes" do
        case result do
          {:ok, %{git_status: status, current_branch: branch}} ->
    ...
    socket =
      if owns? do
        ...
        case {socket.assigns.active_view, socket.assigns.deck_git_queued_project_id} do
          {"changes", _queued} ->
            assign(socket, :deck_git_queued_project_id, nil)

          {_view, queued} when queued == socket.assigns.project.id ->
  ...
  defp complete_deck_git_exit(socket, generation, reason) do
    ...
    socket =
      if owns? and marker.project_id == socket.assigns.project.id and
           socket.assigns.active_view != "changes" do
  ...
  defp refresh_run_summary_facts(socket, runs, ready_results, opts \\ []) do
    ...
    step_summaries =
      opts
      |> Keyword.get(:step_summaries, %{})
      |> load_missing_summary_steps([mission, research], Keyword.get(opts, :load_steps?, true))
    ...
    mission_phase =
      summary_phase(socket, mission, step_summaries, Keyword.get(opts, :preserve_missing?, false))

    summary_steps =
      summary_research_steps(
        socket,
        research,
        step_summaries,
        Keyword.get(opts, :preserve_missing?, false)
      )

    pending_approval_count =
      Keyword.get_lazy(opts, :pending_approval_count, fn ->
        Runs.count_pending_approvals(socket.assigns.session.id)
      end)

    socket
    |> assign(:summary_mission_run, mission)
    |> assign(:summary_mission_phase, mission_phase)
    |> assign(:summary_pending_approvals, pending_approval_count)
  ...
  defp load_missing_summary_steps(step_summaries, _runs, false), do: step_summaries

  defp load_missing_summary_steps(step_summaries, runs, true) do
    runs
    |> Enum.reject(&is_nil/1)
    |> Enum.uniq_by(& &1.id)
    |> Enum.reduce(step_summaries, fn run, summaries ->
      if Map.has_key?(summaries, run.id) do
        summaries
      else
        steps =
        ...
        Map.put(summaries, run.id, steps)
      end
    end)
  end

  defp summary_phase(_socket, nil, _step_summaries, _preserve_missing?), do: nil

  defp summary_phase(socket, mission, step_summaries, preserve_missing?) do
    case Map.fetch(step_summaries, mission.id) do
      {:ok, steps} ->
        phase_from_steps(mission, steps)

      :error ->
        if (preserve_missing? and socket.assigns[:summary_mission_run]) &&
             socket.assigns.summary_mission_run.id == mission.id,
           do: socket.assigns[:summary_mission_phase],
           else: phase_from_steps(mission, [])
    end
  end

  defp summary_research_steps(_socket, nil, _step_summaries, _preserve_missing?), do: []

  defp summary_research_steps(socket, research, step_summaries, preserve_missing?) do
    case Map.fetch(step_summaries, research.id) do
      {:ok, steps} ->
        steps

      :error ->
        if (preserve_missing? and socket.assigns[:summary_research_run]) &&
             socket.assigns.summary_research_run.id == research.id,
           do: socket.assigns[:research_summary_steps] || [],
           else: []
    end
  end
  ...
  defp workspace_path(:root, "research"), do: research_path(:root)
  defp workspace_path(:root, view), do: ~p"/?view=#{view}"
  defp workspace_path({:session, id}, "deck"), do: ~p"/sessions/#{id}"
  defp workspace_path({:session, id}, "research"), do: research_path({:session, id})
  ...
  defp activate_workspace_view_change(socket, "changes", "changes") do
    socket
    |> assign(:pending_git_confirmation, nil)
    |> refresh_git_state()
  end
  ...
  defp activate_workspace_view_change(socket, previous_view, view) do
    ...
    socket =
      if previous_view == "changes" and view != "changes",
        do: assign(socket, :pending_git_confirmation, nil),
        else: socket
  ...
  defp safe_get_run(id) when is_binary(id) do
    Runs.get_run(id)
  rescue
    _ in [Ecto.Query.CastError] -> nil
  end

  defp safe_get_run(_id), do: nil
  ...
  defp execute_command_palette_item(socket, %{
    ...
    socket |> assign(:show_command_palette, false) |> perform_git_fetch()
  end
  ...
  defp refresh_git_state(socket) do
    ...
    with {:ok, status} <- Git.status(root, path_limit: 500, output_limit_bytes: 1 * 1_024 * 1_024) do
      ...
      status = Map.put(status, :branch, current_branch)
      ...
      active_paths = Enum.map(active_list, &diff_path/1)

      selected_diff_file =
        if socket.assigns[:selected_diff_file] in active_paths,
          do: socket.assigns[:selected_diff_file],
          else: List.first(active_paths)
  ...
  defp clear_git_detail_state(socket) do
    socket
    |> assign(:git_status, nil)
    |> assign(:git_branches, [])
    |> assign(:current_branch, "main")
  ...
  defp git_scope("staged"), do: :staged
  defp git_scope("unstaged"), do: :unstaged
  defp git_scope("untracked"), do: :untracked
  defp git_scope(:staged), do: :staged
  defp git_scope(:unstaged), do: :unstaged
  defp git_scope(:untracked), do: :untracked
  defp git_scope(_scope), do: nil

  defp diff_path(diff), do: Map.get(diff, :path) || Map.get(diff, :new_path)

  defp authorize_diff_path(socket, requested_path, scope)
       when scope in [:staged, :unstaged] and is_binary(requested_path) do
    rows =
      if socket.assigns.active_view == "changes" do
        if(scope == :staged, do: socket.assigns.staged_diffs, else: socket.assigns.unstaged_diffs)
      else
        []
      end

    case Enum.find(rows, &(diff_path(&1) == requested_path)) do
      nil -> {:error, :unauthorized_path}
      row -> resolve_accepted_path(socket, diff_path(row))
    end
  end

  defp authorize_diff_path(_socket, _requested_path, _scope),
    do: {:error, :unauthorized_path}

  defp authorize_selected_diff(socket, scope, requested_path) do
    with true <- socket.assigns.active_diff_scope == scope,
         true <- socket.assigns.selected_diff_file == requested_path,
         true <- socket.assigns.diff_file_path == requested_path,
         {:ok, accepted_path} <- authorize_diff_path(socket, requested_path, scope) do
      {:ok, accepted_path}
    else
      _ -> {:error, :unauthorized_diff}
    end
  end

  defp authorize_hunk(socket, scope, requested_path, hunk_id)
       when scope in [:staged, :unstaged] and is_binary(hunk_id) do
    with {:ok, accepted_path} <- authorize_selected_diff(socket, scope, requested_path),
         true <- Enum.any?(socket.assigns.diff_hunks, &(Map.get(&1, :id) == hunk_id)) do
      {:ok, accepted_path}
    else
      _ -> {:error, :unauthorized_hunk}
    end
  end

  defp authorize_hunk(_socket, _scope, _requested_path, _hunk_id),
    do: {:error, :unauthorized_hunk}

  defp authorize_status_path(socket, requested_path, allowed_scopes)
       when is_binary(requested_path) and is_list(allowed_scopes) do
    accepted =
      if socket.assigns.active_view == "changes" do
        allowed_scopes
        |> Enum.flat_map(&status_paths(socket.assigns.git_status, &1))
        |> Enum.find(&(&1 == requested_path))
      end

    if accepted,
      do: resolve_accepted_path(socket, accepted),
      else: {:error, :unauthorized_path}
  end

  defp authorize_status_path(_socket, _requested_path, _allowed_scopes),
    do: {:error, :unauthorized_path}

  defp status_paths(nil, _scope), do: []
  defp status_paths(status, :untracked), do: Map.get(status, :untracked, [])

  defp status_paths(status, :all) do
    status_paths(status, :staged) ++
      status_paths(status, :unstaged) ++ status_paths(status, :untracked)
  end

  defp status_paths(status, scope) when scope in [:staged, :unstaged] do
    status
    |> Map.get(scope, [])
  ...
  defp status_paths(_status, _scope), do: []

  defp resolve_accepted_path(socket, accepted_path) do
    with false <- git_pathspec_magic?(accepted_path),
         {:ok, _canonical_path} <-
           WorkspacePath.resolve(socket.assigns.project.root_path, accepted_path) do
      {:ok, accepted_path}
    else
      true -> {:error, :git_pathspec_magic}
      {:error, reason} -> {:error, reason}
    end
  end
  ...
  defp bounded_exact_diff(root, path, scope) do
  ...
  defp fresh_hunk_snapshot(socket, scope, path, hunk_id) do
    root = socket.assigns.project.root_path
    ...
    with {:ok, status} <-
         ...
         {:ok, raw_diff} <- bounded_exact_diff(root, path, scope),
         true <- String.trim(raw_diff) != "",
         {:ok, file_diffs} <- DiffParser.parse(raw_diff),
         file_diff when not is_nil(file_diff) <- Enum.find(file_diffs, &(diff_path(&1) == path)),
         hunk when not is_nil(hunk) <- Enum.find(file_diff.hunks, &(Map.get(&1, :id) == hunk_id)) do
      fingerprint =
        semantic_digest(%{
          project_id: socket.assigns.project.id,
          session_id: socket.assigns.session.id,
    ...
    with true <- socket.assigns.active_diff_scope == scope,
      ...
      {:ok,
       semantic_digest(%{
         project_id: socket.assigns.project.id,
         session_id: socket.assigns.session.id,
         branch: Map.get(socket.assigns.git_status || %{}, :branch),
         scope: scope,
         path: path,
         hunk_id: hunk_id,
         identity: identity,
  ...
  defp confirm_git_hunk(socket, expected_kind) do
    case socket.assigns.pending_git_confirmation do
        ...
        result =
          with_ui_mutation_lock(socket, fn ->
            with {:ok, accepted_path} <- authorize_status_path(socket, path, [scope]),
                 {:ok, snapshot} <- fresh_hunk_snapshot(socket, scope, accepted_path, hunk_id),
                 true <- snapshot.fingerprint == fingerprint do
      ...
      _ ->
        {:noreply, assign(socket, :pending_git_confirmation, nil)}
    end
  end

  defp compatibility_git_guard(socket, message) do
    case with_ui_mutation_lock(socket, fn -> {:error, :confirmation_required} end) do
      {:error, {:workspace_lock_waiting, locks}} ->
        {:noreply, put_flash(socket, :error, ui_mutation_error({:workspace_lock_waiting, locks}))}
        ...
        case result do
          {:ok, _} ->
            ...
            {:noreply,
             ...
             |> assign(:pending_git_confirmation, nil)
             |> refresh_git_state()
             |> put_flash(:info, git_revert_success_message(scope, path))}
  ...
  defp maybe_reload_reverted_editor(socket, file) do
    if socket.assigns.selected_file == file do
      full_path = Path.join(socket.assigns.project.root_path, file)
  ...
  defp git_revert_success_message(:all, path), do: "Reverted all uncommitted changes in #{path}"
  defp git_revert_success_message(:staged, path), do: "Reverted staged index changes in #{path}"
  defp git_revert_success_message(:unstaged, path), do: "Reverted working-tree changes in #{path}"
  defp git_revert_success_message(:untracked, path), do: "Removed untracked #{path}"

  defp git_path_digest(scope, path) do
    :sha256
    |> :crypto.hash(scope <> ":" <> path)
    |> Base.encode16(case: :lower)
    |> binary_part(0, 12)
  end

  defp git_pathspec_magic?(path) do
  ...
  defp git_confirmation_host(%{kind: :revert_git_file, scope: :all}),
    do: {"git-file-revert-confirmation", "Revert entire file?", "revert_file", nil}

  defp git_confirmation_host(%{kind: :revert_git_file, scope: :staged}),
    do: {"git-file-revert-confirmation", "Revert staged changes?", "revert_file", nil}

  defp git_confirmation_host(%{kind: :revert_git_file}),
    do: {"git-file-revert-confirmation", "Revert working-tree changes?", "revert_file", nil}
  ...
  defp git_confirmation_success_return_id(%{kind: :revert_git_file}),
    do: "changes-staging-title"

  defp git_confirmation_success_return_id(%{kind: kind})
       when kind in [:discard_git_hunk, :revert_git_hunk],
       do: "changes-diff-panel"

  defp git_confirmation_copy(%{kind: :revert_git_file, scope: :all, path: path}) do
    bounded_path = path |> String.replace_invalid() |> String.slice(0, 160)

    "This restores both the index and working tree for #{bounded_path} to HEAD after revalidation."
  end

  defp git_confirmation_copy(%{kind: :revert_git_file, scope: :staged, path: path}) do
    bounded_path = path |> String.replace_invalid() |> String.slice(0, 160)

    "This restores only the staged index layer for #{bounded_path}; working-tree bytes are preserved."
  end

  defp git_confirmation_copy(%{kind: :revert_git_file, scope: :untracked, path: path}) do
    bounded_path = path |> String.replace_invalid() |> String.slice(0, 160)
    "This removes the untracked lexical path #{bounded_path} without following a final symlink."
  end

  defp git_confirmation_copy(%{kind: :revert_git_file, path: path}) do
    bounded_path = path |> String.replace_invalid() |> String.slice(0, 160)

    "This restores only working-tree changes for #{bounded_path}; the staged index layer is preserved."
  end
  ...
  defp assign_session_run_projection(%{assigns: %{active_view: "research"}} = socket, _session_id) do
    socket
    |> clear_selected_run_projection()
    |> refresh_run_rows_preserving_selection()
  end

  defp assign_session_run_projection(socket, session_id),
    do: assign_run_projection(socket, session_id)
  ...
  defp scoped_research_results(results, runs) do
    run_ids = runs |> Enum.map(& &1.id) |> MapSet.new()
  ...
  defp research_workbench_status(%Runs.Run{status: status}, _results),
    do: "Research #{String.capitalize(to_string(status))}"

  defp select_research_projection(socket, %Runs.Run{} = run) do
    select_research_projection(socket, run, bounded_research_steps(run))
  end

  defp select_research_projection(socket, %Runs.Run{} = run, {steps, projection_state}) do
    socket
  ...
  defp refresh_research_workbench(socket) do
    snapshot = research_notification_snapshot(socket)

    selected_id =
      socket.assigns[:selected_research_run] && socket.assigns.selected_research_run.id

    selected =
      Enum.find(snapshot.research_runs, &(&1.id == selected_id)) ||
        List.first(snapshot.research_runs)

    progress = bounded_research_steps(selected)
    step_summaries = if(selected, do: %{selected.id => elem(progress, 0)}, else: %{})

    socket
    |> assign_research_snapshot(snapshot,
      step_summaries: step_summaries,
      load_steps?: true,
      preserve_missing?: false
    )
    |> assign_selected_research_progress(selected, progress)
  end

  defp refresh_research_run_notification(socket, hinted_id, mode) do
    snapshot = research_notification_snapshot(socket)
    affected = Enum.find(snapshot.runs, &(&1.id == hinted_id))

    selected_id =
      socket.assigns[:selected_research_run] && socket.assigns.selected_research_run.id

    durable_selected = Enum.find(snapshot.research_runs, &(&1.id == selected_id))

    selected =
      case {mode, affected} do
        {:created, %Runs.Run{kind: "deep_research"} = created} ->
          created

        _other ->
          durable_selected || List.first(snapshot.research_runs)
      end

    refresh_progress? =
      (match?(%Runs.Run{kind: "deep_research"}, affected) and selected) &&
        affected.id == selected.id

    progress = if(refresh_progress?, do: bounded_research_steps(selected), else: nil)

    step_summaries =
      cond do
        progress ->
          %{selected.id => elem(progress, 0)}

        summary_target?(snapshot.runs, affected) ->
          %{affected.id => affected |> bounded_research_steps() |> elem(0)}
    ...
    socket =
      socket
      |> maybe_sync_research_linked_task(mode, affected)
      |> assign_research_snapshot(snapshot,
        step_summaries: step_summaries,
        load_steps?: false,
    ...
    assign_selected_research_progress(socket, selected, progress)
  end

  defp research_notification_snapshot(socket) do
    runs = Runs.list_runs(session_id: socket.assigns.session.id, limit: 100)
  ...
  defp assign_research_snapshot(socket, snapshot, opts) do
    socket
    |> assign(:run_rows, snapshot.runs)
    |> assign(:run_count, length(snapshot.runs))
    |> assign(:run_counts, run_counts(snapshot.runs, snapshot.pending_approval_count))
    |> assign(:run_dispatcher_stats, safe_dispatcher_stats())
    |> assign(:research_runs, snapshot.research_runs)
    |> assign(:research_results, snapshot.ready_results)
    |> refresh_run_summary_facts(snapshot.runs, snapshot.ready_results,
      step_summaries: Keyword.get(opts, :step_summaries, %{}),
      load_steps?: Keyword.get(opts, :load_steps?, false),
      preserve_missing?: Keyword.get(opts, :preserve_missing?, true),
      pending_approval_count: snapshot.pending_approval_count
    )
    |> rebuild_instrument_summaries()
  end

  defp assign_selected_research_progress(socket, nil, _progress) do
    socket
    |> assign(:selected_research_run, nil)
  ...
  defp maybe_sync_research_linked_task(socket, :updated, %Runs.Run{} = run),
    do: sync_run_linked_task(socket, run)

  defp maybe_sync_research_linked_task(socket, _mode, _run), do: socket

  defp summary_target?(_runs, nil), do: false

  defp summary_target?(runs, affected) do
    mission = select_active_mission(runs)
    research = Enum.find(runs, &(&1.kind == "deep_research"))

    Enum.any?([mission, research], &(&1 && &1.id == affected.id))
  end
  ...
  defp refresh_research_evidence(socket) do
    ...
    summary_research =
      case socket.assigns[:summary_research_run] do
        %{id: id} -> Enum.find(research, &(&1.id == id)) || List.first(research)
        _missing -> List.first(research)
      end

    summary_result =
      if summary_research,
        do: Enum.find(ready, &(&1.run_id == summary_research.id)),
        else: nil

    level =
      get_in((summary_research && summary_research.metadata) || %{}, ["research", "level"]) ||
        (summary_result && summary_result.level)

    socket
    |> assign(:research_runs, research)
    |> assign(:research_results, ready)
    |> assign(:summary_research_run, summary_research)
    |> assign(:summary_research_result, summary_result)
    |> assign(:summary_research_level, level)
    |> rebuild_instrument_summaries()
  end
